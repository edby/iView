  ...
  <div v-title="title">
    ...
    <div class="mui-scroll-wrapper mui-slider-indicator mui-segmented-control mui-segmented-control-inverted">
      <div class="mui-scroll">
        ...
        <router-link to="/user/order/orderpending" class="mui-control-item mui-active" replace>待付款</router-link>
        ...
        <router-link to="/user/order/ordercompleted" class="mui-control-item" replace>已完成</router-link>
    ...
    <div>
      ...
      <Row class="order_list" v-for="(v,k) in items" :key="k" :id="v.id" v-show="showss">
        ...
        <Col span="24">
        <button type="button" class="pay" @click.stop="pay(v.id)">立即付款</button>
        <button type="button" class="cancel" @click.stop="cancel(v.id,)">取消订单</button>
        </Col>
      </Row>
    </div>
  </div>
...
export default {
  ...
  created() {
    ...
    this.$axios.get("/user/order/index?status=1").then(res => {
      //console.log(res)
  ...
  methods: {
    ...
    cancel(id) {
      this.$axios.get("/user/order-goods/close?id=" + id).then(res => {
        alert("取消成功！");
        setTimeout(() => {
    ...
    pay(id) {
      this.$router.push("/order/ispay?id=" + id);
    },
     index() {
      this.$router.push({ name: "index" });
    }
